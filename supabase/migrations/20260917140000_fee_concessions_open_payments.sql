-- Only open or settled payment attempts count as payment activity; failed/expired
-- attempts moved no money. Unknown statuses still count (fail safe).
...
begin
  ...
  loop
    ...
    v_activity := v_paid > 0
      ...
      or exists (
        select 1
        from public.payment_transaction_items pti
        join public.payment_transactions pt on pt.id = pti.transaction_id
        where pti.bill_id = v_sb.id
          and pt.status is distinct from 'failed'
          and pt.status is distinct from 'expired');
