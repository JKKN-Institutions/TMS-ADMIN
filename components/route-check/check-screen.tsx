...
import { fetchCheck, removeEntry } from '@/app/boarding/route-check/route-check-api';
...
/**
 * The inspection screen for one check: header, counts, riders / staff, scan log,
 * Scan / Finish. Rendered by the staff app (/boarding/route-check/[checkId]) and,
 * `embedded` inside the admin layout, by /inspections/run/[checkId].
 */
export function RouteCheckScreen({ checkId, embedded = false }: { checkId: string; embedded?: boolean }) {
  // Standalone (staff app) the screen owns its width and padding; embedded, the
  // admin layout already provides both.
  const shell = embedded ? 'w-full' : `${PAGE} p-4`;
  ...
  const scanButton = (cls: string) => (
    <button
      type="button"
      onClick={() => {
        // iPhones only speak after a tap: unlock the scan voice now, from the tap.
        primeSpeech();
        setScanOpen(true);
      }}
  ...
  return (
    <div className={`${shell} space-y-4 pb-28 md:pb-6 ${embedded ? '' : 'lg:p-6'}`}>
      {/* Check details */}
