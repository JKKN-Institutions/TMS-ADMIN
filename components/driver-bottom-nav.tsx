...
// items become the bar shortcuts; everything (including coming-soon) is listed in the
// "More" sheet. In driver v1 only Dashboard is live, so the bar is Dashboard + More.
const PRIMARY_HREFS = [
  '/driver/dashboard',
  '/driver/routes',
  '/driver/passengers',
...
/**
 * Mobile-only bottom navigation for the driver portal (lg:hidden). Replaces the
 * sidebar on small screens: up to 4 quick items + a "More" bottom sheet that lists
 * every destination in a 3-column icon grid. Mirrors the student/admin BottomNav so
 * the shells stay visually in lock-step, driven by the flat driverNavigation list.
 * Pages flagged `comingSoon` render disabled.
 */
export default function DriverBottomNav() {
  ...
  const byHref = new Map(driverNavigation.map((i) => [i.href, i]));

  // Build the primary shortcuts: priority hrefs first (live only), then fill from any
  // remaining live items.
  const primary: DriverNavItem[] = [];
  ...
  if (primary.length < 4) {
    for (const item of driverNavigation) {
