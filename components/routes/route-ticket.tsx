...
import {
  ...
  ArrowRight, CircleDot, Route as RouteIcon,
} from 'lucide-react';
...
function Endpoint({
  label, place, time, icon: Icon, align = 'left',
}: {
  label: string; place: string; time: string; icon: ComponentType<{ className?: string }>; align?: 'left' | 'right';
}) {
  return (
    <div className={cn('flex-1 min-w-0', align === 'right' && 'text-right')}>
      <div className={cn('flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wide text-white/70', align === 'right' && 'justify-end')}>
...
function Tag({ children, tone }: { children: ReactNode; tone: 'green' | 'indigo' | 'gray' | 'solid' }) {
  ...
  return (
    <span className={cn('rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide', tones[tone])}>{children}</span>
  );
}
...
/**
 * Shared route-info "ticket". Renders a route's hero journey strip, stat strip,
 * stops timeline, and driver/vehicle sidebar.
 *
 * - `highlightStopId`: the viewer's personal boarding stop (student portal). When
 *   null (boarding staff supervise the whole route, no personal stop) the "Your
 *   stop" highlight, legend item, and sidebar card are all omitted.
 * - `actions`: optional buttons rendered in the hero (e.g. boarding's roster/scan).
 */
export function RouteTicket({
  route, highlightStopId = null, actions,
}: {
  route: RouteDetail;
  ...
  const boardingStop = highlightStopId ? stops.find((s) => s.id === highlightStopId) ?? null : null;
          ...
          <div className="relative space-y-6">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <h1 className="mt-2 text-xl sm:text-2xl font-bold text-white leading-tight">{route.routeName ?? '—'}</h1>
              </div>
              <span className={cn('shrink-0 rounded-full px-3 py-1 text-xs font-semibold capitalize backdrop-blur', STATUS_TONE[statusKey] ?? 'bg-white/15 text-white ring-1 ring-white/30')}>
                {cap(route.status)}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-5">
              <Endpoint label="From" place={route.startLocation ?? '—'} time={fmtTime(route.departureTime)} icon={MapPin} />
              ...
              <div className="hidden sm:flex flex-col items-center justify-center min-w-[96px]">
                ...
                <span className="mt-2 text-[10px] font-medium uppercase tracking-wide text-white/80">{fmtDuration(route.duration)}</span>
              </div>
              <Endpoint label="To" place={route.endLocation ?? '—'} time={fmtTime(route.arrivalTime)} icon={Flag} align="right" />
            </div>
        ...
        <section className="order-2 rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 lg:order-1 lg:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 px-4 py-4 dark:border-gray-800 sm:px-6">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-300">{stops.length}</span>
            </div>
          ...
          <div className="px-4 py-5 sm:px-6">
            {stops.length === 0 ? (
              ...
              <ol className="relative">
                {stops.map((s, i) => {
                  const isBoarding = highlightStopId != null && s.id === highlightStopId;
                  const isFirst = i === 0;
                  const isLast = i === stops.length - 1;
                  return (
                    <li key={s.id} className="relative flex gap-3 pb-7 last:pb-0 sm:gap-4">
                      {!isLast && <span className="absolute left-4 top-8 bottom-0 w-0.5 -translate-x-1/2 bg-gray-200 dark:bg-gray-700" />}
                      <span
                        className={cn(
                      ...
                      <div
                        className={cn(
                          ...
                          isBoarding ? 'bg-green-50 dark:bg-green-950/30' : 'hover:bg-gray-50 dark:hover:bg-gray-800/50'
                        )}
                      >
                        <div className="min-w-0">
                          <p
        ...
        <div className="order-1 grid content-start gap-6 sm:grid-cols-2 lg:order-2 lg:grid-cols-1">
          {boardingStop && (
            <div className="flex items-center gap-4 rounded-xl border border-green-200 bg-green-50 p-5 shadow-sm dark:border-green-900/50 dark:bg-green-950/30">
              ...
              <div className="min-w-0">
                <p className="text-[11px] font-medium uppercase tracking-wide text-green-700/80 dark:text-green-300/70">Your boarding stop</p>
                <p className="truncate text-base font-semibold text-green-800 dark:text-green-200">{boardingStop.name}</p>
                <p className="text-xs text-green-700/80 dark:text-green-300/80">Pickup {fmtTime(boardingStop.time)} · Drop {fmtTime(boardingStop.eveningTime)}</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="min-w-0">
              <p className="text-[11px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">Driver</p>
              <p className="truncate text-base font-semibold text-gray-900 dark:text-white">{route.driverName ?? 'Not assigned'}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="min-w-0">
              <p className="text-[11px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">Vehicle</p>
              <p className="truncate text-base font-semibold text-gray-900 dark:text-white">{route.vehicle ? route.vehicle.registrationNumber : 'Not assigned'}</p>
...
export function RouteNotice({
  ...
  tone: 'amber' | 'red'; icon: ComponentType<{ className?: string }>; title: string; body: string;
}) {
  const tones = {
