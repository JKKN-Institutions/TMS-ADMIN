...
async function getRoutes(_request: NextRequest, auth: AuthContext) {
  try {
    ...
    // Allocated rider count per route (same roster filter as /api/driver/passengers).
    // tms_route.current_passengers is stale (0 everywhere), so we count the real allocation.
