...
import { fetchGroupSummary } from '@/lib/reports/rpc';
import { groupSheetRows, reportFileName } from '@/lib/reports/sheets';
import { xlsxResponse } from '@/lib/reports/workbook';

/**
 * GET /api/admin/reports/institutions — roll-up by institution, or by
 * department with `group=department`. One row per group, never paginated.
 */
...
async function report(request: NextRequest, auth: AuthContext) {
  try {
    ...
    const rows = await fetchGroupSummary(svc, params);
    const label = params.group === 'department' ? 'Department' : 'Institution';
    ...
    if (params.format === 'xlsx') {
      ...
      return xlsxResponse(
        label === 'Department' ? 'Departments' : 'Institutions',
        groupSheetRows(rows, label),
        reportFileName(`transport-${params.group}s`, params.from, params.to),
      );
    }

    return NextResponse.json({
      success: true,
      data: { rows, group: params.group, range: { from: params.from, to: params.to, direction: params.direction } },
    });
  } catch (e) {
    console.error('admin reports institutions error:', e);
