...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const studentResponse = await fetch('https://www.jkkn.ai/api/api-management/students', {
        method: 'GET',
        headers: {
      ...
      if (!foundStudent) {
        const staffResponse = await fetch('https://www.jkkn.ai/api/api-management/staff', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${apiKey}`
        ...
        if (staffResponse.ok) {
          ...
          foundStudent = staff.find((extStaff: any) => {
