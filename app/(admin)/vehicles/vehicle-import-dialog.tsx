...
import { downloadVehicleTemplate } from './vehicle-export';
...
// The Vehicles page holds its list in local state (not React Query), so the
// parent passes `onImported` to refetch after a successful upload.
export function VehicleImportDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (json) => {
      setResult(json);
      onImported?.();
      const ok = json.created + json.updated;
      if (json.failed === 0) toast.success(`Imported ${ok} vehicle(s) — ${json.created} created, ${json.updated} updated`);
      else toast(`Imported ${ok}, ${json.failed} failed`, { icon: '⚠️' });
    },
    ...
    <Dialog open={open} onOpenChange={(o) => !mutation.isPending && handleOpenChange(o)}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Creates new vehicles and updates existing ones, matched by{' '}
            <span className="font-medium text-gray-900">registration number</span>. Download the template to see the
            expected columns.
          </DialogDescription>
        </DialogHeader>
              ...
              {errors.length > 0 && (
                <div className="max-h-32 space-y-1 overflow-y-auto text-red-600">
                  {errors.map((e) => (
                    <div key={e.row}>
                      Row {e.row}
                      {e.registration_number ? ` (${e.registration_number})` : ''}: {e.message}
                    </div>
                  ))}
                </div>
              )}
