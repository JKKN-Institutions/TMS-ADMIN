...
export async function generateBills(
  ...
  try {
    ...
    // EXPLICIT empty array is rejected outright BY THE CALLER (see the route):
    // intersectPersonIds reads "no usable ids" as "no scoping applied" (by
    // design, for the absent-field case), so a selection UI that serialises
    // "nothing selected" as [] would otherwise silently bill the ENTIRE cohort
    // instead of nobody. An absent personIds field keeps meaning "bill
    // everyone".
    const requestedPersonIds: string[] | null = opts.personIds ?? null;

    const { data: fs } = await svc.from('tms_fee_structure').select('*').eq('id', id).maybeSingle();
    if (!fs) return { ok: false, status: 404, error: 'Fee structure not found' };
    if (fs.status !== 'active') {
      return { ok: false, status: 400, error: 'Activate the fee structure before generating bills.' };
    }
    ...
    let people = await resolveApplicablePeople(svc, fs);
    ...
    if (isStopWise) {
      ...
      for (let i = 0; i < ids.length; i += CHUNK_STOPS) {
        const { data: rows, error: stopErr } = await svc
          .from(stopTable)
          .select(cols)
          .in('id', ids.slice(i, i + CHUNK_STOPS));
        if (stopErr) {
          return { ok: false, status: 500, error: 'Failed to resolve boarding stops.' };
        }
      ...
      if (isStaff) {
        ...
        for (let i = 0; i < profileIds.length; i += CHUNK_STOPS) {
          ...
          if (profErr) {
            return { ok: false, status: 500, error: 'Failed to resolve staff profile emails.' };
          }
    ...
    if (isStopWise && fs.audience === 'staff') {
      ...
      if (assignErr) {
        ...
        return { ok: false, status: 500, error: 'Failed to load bus in-charge assignments.' };
      }
    ...
    const { data: overrideRows, error: overrideErr } = await svc
    ...
    if (overrideErr) {
      ...
      return { ok: false, status: 500, error: 'Failed to load per-person fee overrides.' };
    }
    ...
    if (resolvedIds.length) {
      ...
      for (let i = 0; i < resolvedIds.length; i += CHUNK) {
        const { data: other, error: conflictErr } = await svc
          .from('tms_fee_bill')
          .select('person_id')
          .eq('transport_year_id', fs.transport_year_id)
        ...
        if (conflictErr) {
          return { ok: false, status: 500, error: 'Failed to check for cross-structure billing conflicts.' };
        }
    ...
    try {
      ...
      if (billedByStaff.size) {
        ...
        for (let i = 0; i < stopIds.length && !lookupFailed; i += CHUNK_NOTIFY) {
          const { data: rows, error: stErr } = await svc
            .from('tms_route_stop')
            .select('id, stop_name')
        ...
        if (!lookupFailed) {
          for (const [staffId, agg] of billedByStaff) {
            ...
            const delivered = await notifyProfile(svc, {
              profileId,
              // Cast, not a coalesce: an automated run passes null and must keep
              // writing null (createdBy), exactly as before the extraction.
